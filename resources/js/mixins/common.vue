...
<script>
    export default {
        ...
        methods: {
            ...
            dateIsBetweenTwoDates (specified_dateYMD, min_dateYMD, max_dateYMD)
            {
                var specified_datetime = (new Date(specified_dateYMD)).getTime();
                var min_datetime = (new Date(min_dateYMD)).getTime();
                var max_datetime = (new Date(max_dateYMD)).getTime();

                return ((specified_datetime >= min_datetime) && (specified_datetime <= max_datetime)) ? 1 : 0;
            },
            ...
            getPaginatedSerial(paginator={}, index=0)
            {
