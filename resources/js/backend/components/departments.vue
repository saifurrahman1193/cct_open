                    ...
                    <v-card-text>
                                            ...
                                            <v-container>
                                                <v-row>
                                                    <v-col cols="12" sm="12" md="12">
                                                        <v-text-field name="department" label="Department Name*" v-model="department.department"
                                                            :rules="departmentRules"
                                                            :error-messages="departmentError.department"
                                                        ></v-text-field>
                                                    </v-col>
                                                </v-row>
                                            </v-container>
                        ...
                        >
                            ...
                            <template v-slot:[`item.action`]="{ item }">
                                ...
                                <v-tooltip top>
                                    <template v-slot:activator="{ on }">
                                        <v-icon @click="editDialog(item.departmentId)" v-on="on">edit</v-icon>
                                    </template>
                                ...
                                <!-- <v-tooltip top >
                                    <template v-slot:activator="{ on }">
                                        <v-icon @click="deleteConfirm(item.departmentId)" v-on="on">delete</v-icon>
                                    </template>
...
<script>
    ...
    export default {
        ...
        methods: {
            getDepartments(){
                var _this = this;
                axios.get('/api/getDepartments'+'?token='+this.token)
            ...
            clearForm(){
                ...
                this.departmentError= { department: '' }
            },

            adding(){
                console.log(this.department)
                var _this=this
                axios.post('/api/addDepartment'+'?token='+this.token, this.department)
                .then(function (response) {
                ...
                .catch(function (error) {
                    _this.e_alert = true;
                    _this.departmentError = { department: error.response.data.department };
                })
            },
            ...
            deleting(){
                ...
                var _this = this;
                axios.post('/api/deleteDepartment/'+this.deletingId+'?token='+this.token)
                .then(function (response) {
                    _this.getDepartments()
            ...
            setEditDialogFields(id){
                var _this = this;
                axios.get('/api/getDepartment/'+id+'?token='+this.token)
                .then(function (response) {
            ...
            editing(){
                var _this = this;
                axios.post('/api/editDepartment?token='+this.token, this.department)
               .then(function (response) {
                    _this.s_alert = true;
            ...
            departmentDataUpdate(department){
                this.getDepartments()
            }
